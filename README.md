...
1. **📂 카테고리 그룹 선택**: 
   ...
   - 각 그룹의 논문 수 표시
   - 세부 카테고리 목록 확인 (한글 병기)
2. **연도 범위 선택**: 분석할 연도 범위를 슬라이더로 선택
3. **Top N 키워드 개수**: 분석할 상위 키워드 개수 선택 (5~30개)
4. **키워드 추출 소스**: Abstract(초록) 또는 Title(제목)에서 키워드 추출 선택
5. **🚀 분석 시작**: 선택한 설정으로 분석 시작
6. **데이터 요약**: 총 논문 수, 선택된 논문 수, 연도 범위 표시
7. **⚙️ 설정**:
   - **🌐 언어 선택**: 한국어 / English 중 선택 가능
     - 언어 변경 시 모든 메뉴, 차트, 메시지가 선택한 언어로 즉시 변경됩니다
